...
class AudioEngine:
    ...
    def create_track(self, wav_sample, bpm):
        source_track = AudioSourceTrack(self.output_stream, wav_sample, bpm, self.SAMPLE_RATE)
        source_track.start()
        return source_track

    def create_mixer(self, all_wav_samples, bpm, nb_steps, on_current_step_changed):
        source_mixer = AudioSourceMixer(self.output_stream, all_wav_samples, bpm, self.SAMPLE_RATE, nb_steps, on_current_step_changed)
        source_mixer.start()
        return source_mixer
